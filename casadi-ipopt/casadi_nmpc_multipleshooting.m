...
g = [g; x0 - P(M*M + 1:end)]; % initial condition constraints
for k = 1:N
    xNext = X(:,k + 1);  
    uk = U(:,k);
...
%     rhs = X(:,k) + Ts * ffunc(X(:,k), uk);
    g = [g; rhs - xNext];
    % objective
    L = 0.5*(xNext-xRef).'*Q*(xNext-xRef) + 0.5*(uk-uRef).'*R*(uk-uRef);
    obj = obj + L;
end
%%
...
opts.print_time = 0;
opts.ipopt.acceptable_tol = 1e0;
opts.ipopt.acceptable_obj_change_tol = 1e0;
...
args.ubg(1:nx*(N+1)) = 0;
% lower and upper
args.lbx(1:nx*(N+1) + nu*N,1) = -inf; % opt vars lower bound
args.ubx(1:nx*(N+1) + nu*N,1) = inf; % opt vars input upper bound
args.lbx(nx*(N+1) + 1:end) = Ta;
args.ubx(nx*(N+1) + 1:end) = Ta + 400;
% parameters
x0 = (Ta)*ones(nx,1); % initial state
ref = [TRef; uRef]; % ref
args.p   = [ref;x0]; % set the values of the parameters vector
% initial guess
args.x0  = [repmat(x0, N+1,1); repmat((Ta + 200) * ones(nu, 1), N,1)];
% solve
tic
...
toc
% get solution
uOptimal = reshape(full(sol.x(nx*(N+1)+1:end)).', nu, N); 
xOptimal = reshape(full(sol.x(nx+1:nx*(N+1))).', nx, N); 
